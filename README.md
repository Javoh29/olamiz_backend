...
**Olamiz** (olamiz.uz — клиенты: app, SEO-витрина) и **Beramiz** (beramiz.uz — прокатчики:
Telegram-бот, лендинг онбординга).
...
Без Docker (напр. Postgres через Homebrew): `brew install postgresql@16 && brew services start postgresql@16`,
затем создать роль и БД под дефолтный `DATABASE_URL`:

```bash
psql -d postgres -c "CREATE ROLE olamiz LOGIN PASSWORD 'olamiz' CREATEDB;"
createdb -O olamiz olamiz
```
